...
public class CtSph implements Sph {
    ...
    /**
     *@描述
     *@参数 resourceWrapper 资源对象
            count 默认值为1
            prioritized 默认值为false
            args
     *@返回值
     *@创建人 wrc
     *@创建时间 2022/4/26
    ...
    private Entry entryWithPriority(ResourceWrapper resourceWrapper, int count, boolean prioritized, Object... args)
        throws BlockException {
        // 从ThreadLocal中获取一个context
        // 一个请求占用一个线程，一个线程绑定一个context
        Context context = ContextUtil.getContext();
        // 如果context是NullContext类型，即标识当前系统中的context数量已经超出阈值
        // 即访问请求的数量超出了阈值，此时之积分放回一个无需做规则监测的资源操作对象
        if (context instanceof NullContext) {
            // The {@link NullContext} indicates that the amount of context has exceeded the threshold,
            // so here init the entry only. No rule checking will be done.
        ...
        // 如果context为空，创建一个默认的并放入ThreadLocal
        if (context == null) {
            // Using default context.
            context = InternalContextUtil.internalEnter(Constants.CONTEXT_DEFAULT_NAME);
        }
         ...
         */
        // 如果chain为空，则意味着超出了chain的阈值，请求数量超出了阈值，返回一个无需做规则检测的资源操作对象
        if (chain == null) {
            return new CtEntry(resourceWrapper, null, context);
        }
    ...
    ProcessorSlot<Object> lookProcessChain(ResourceWrapper resourceWrapper) {
        // 从缓存map中获取当前资源的slotChain，缓存map的key为资源，value是chain
        ...
        if (chain == null) {
            synchronized (LOCK) {
                ...
                if (chain == null) {
                    // Entry size limit.
                    // 若缓存map的size >= chain数量最大阈值，直接返回null，不再创建新的chain
    ...
    public Entry entryWithType(String name, int resourceType, EntryType entryType, int count, boolean prioritized,
        ...
        //返回一个资源操作对象，
        // 如果prioritized为true，表示当前访问必须按优先级访问，需要等待根据其优先级计算出的时间才可通行
        // 如果prioritized为false，则当前访问不需要等待
        return entryWithPriority(resource, count, prioritized, args);
    }
