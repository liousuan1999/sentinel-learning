...
public class ContextUtil {
    ...
    // 要确定一个context需要从两个参数确定：1名字；2来源
    protected static Context trueEnter(String name, String origin) {
        Context context = contextHolder.get();
        if (context == null) {
            // 如果context为空，则尝试从缓存map中获取context。缓存中key为context名称，value为EntranceNode
            Map<String, DefaultNode> localCacheNameMap = contextNameNodeMap;
            // 获取EntranceNode
            DefaultNode node = localCacheNameMap.get(name);
            if (node == null) {
                // 若缓存map的size大于context数量的最大阈值，则返回NULL_CONTEXT
                    ...
                    try {
                        ...
                        if (node == null) {
                            if (contextNameNodeMap.size() > Constants.MAX_CONTEXT_NAME_SIZE) {
                                ...
                                // 将新建node写入到缓存map
                                // 写时复制，（用空间换效率），为了防止“迭代稳定性问题”--iterate stable
                                // “迭代稳定性问题”，对一个共享集合的写操作，最好用写时复制，否则会引发读取脏数据的问题
